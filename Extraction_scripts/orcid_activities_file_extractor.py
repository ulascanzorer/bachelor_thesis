...
class Subfolders_Processor:
    ...
    def process_entries(self):
        ...
        for entry in self.entry_buffer:
            all_orcids.append(entry["orcid_id"])

        orcid_to_names = {} # e.g. { "0000-0003-4496-9510": ["Chao", "Tao"], ... }

        found_authors_cursor = self.db.authors.find({ "orcid_id": { "$in": all_orcids } })
        ...
        for found_author in found_authors_cursor:
            ...
            orcid_to_names[orcid_id] = [given_names, family_name]
        ...
        for entry in self.entry_buffer:
            ...
            if entry["orcid_id"] in orcid_to_names:
                given_names = orcid_to_names[entry["orcid_id"]][0]
                family_name = orcid_to_names[entry["orcid_id"]][1]
    ...
    def process_works_folder(self, works_folder):
        ...
        entry = {
            "orcid_id": works_folder.parent.name,
            "works": [],
            "given_names": "",
            "family_name": "",
        }
            ...
            with open(work_xml_file, "r") as xml_file:
                ...
                if "work:contributors" in work_dict and work_dict["work:contributors"] is not None and "work:contributor" in work_dict["work:contributors"]:
                    ...
                    # If we have more than one contributors.
                        ...
                        if contributor_entries is not None:
                            for contributor_entry in contributor_entries:
                                if contributor_entry is not None and "work:credit-name" in contributor_entry and contributor_entry["work:credit-name"] is not None:
...
if __name__ == "__main__":
    ...
    all_subfolders = list(activities_path.iterdir())
    ...
    for chunk in divide_list_into_chunks(all_subfolders, 10):
        actor = Subfolders_Processor.remote()
    ...
    ### DON'T FORGET TO REMOVE DUPLICATES MANUALLY AFTERWARDS! ###
    ### AND ALSO ADD THE GENDERS! ###
